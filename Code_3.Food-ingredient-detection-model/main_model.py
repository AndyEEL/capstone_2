...
'''LocFLS에 사용되는 layer (local info extract)'''

    # A : stream1개 적고 AvgPooling 씀
    # B : stream1개 많고 MaxPooling

class InceptionA(nn.Module):

    def __init__(self, in_channels, out_channels):
        super(InceptionA, self).__init__() # It's for backward compatibility
    ...
    def forward(self, x):
        ...
        return y
        # return output with channel of 2
...
class InceptionB(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.stream2 = nn.Sequential(
            ConvBlock(in_channels, mid_channels, 1),
            ConvBlock(mid_channels, mid_channels, 3, p=1), # same size
            ConvBlock(mid_channels, mid_channels, 3, s=2, p=1),
        )
        self.stream3 = nn.Sequential(
...
'''
GloFL - attention module 
Attention module in space
'''
...
class SpatialAttn(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.mean(1, keepdim=True) # changing from hwc to hw1
        # 3-by-3 conv
        h = x.size(2)
        x = self.conv1(x)
        # bilinear resizing
        ...
        x = self.conv2(x)
        return x  
        ## return : (h*w*1)^2 soft map

'''
Attention module on the channel
'''
class ChannelAttn(nn.Module):
    ...
    def forward(self, x):
        # Squeeze & Excitation 과정 실행 -> Global한 정보 추출 가능
        # squeeze operation (global average pooling)
        x = F.avg_pool2d(x, x.size()[2:]) #The output is 1*1*c
        # excitation operation (2 conv layers)
        x = self.conv1(x)
        x = self.conv2(x)
        return x
        # What is returned is a map of 1*1*c

'''
Attention fusion on space and channel
It is the attention on the space and the channel to do a matrix multiplication
'''
...
class HardAttn(nn.Module):
    ...
    def forward(self, x):
        ...
        return theta
         #  What is returned is 2T (T is the number of regions ).
        #  Because the scale will be fixed. So just learn the value of displacement
...
class MODEL(nn.Module):
    ...
    # purpose : generates attentional regions (e.g., ingredient relevant regions) from different regions via cascaded spatial transformers

    # spatial transformer ( LocFLS )
    ...
    def forward(self, x):
        ...
        x2_out = x2 * x2_attn # attention 가중치를 x2 feature에 곱해주면서 theta 반응 활성화
 
        if self.multi_scale:
            #  attention global layer1  avg pooling 
            x2_avg = F.adaptive_avg_pool2d(x2_out, (1, 1)).view(x2_out.size(0), -1)  #512 vector
        ...
        if self.learn_region:
            ...
            for region_idx in range(4):
                ...
                x2_local_i = F.avg_pool2d(x2_local_i, x2_local_i.size()[2:]).view(x2_local_i.size(0), -1) #Global pooling for each local
                local_512[:,region_idx] = x2_local_i

                x3_local_i = x3_local_list[region_idx]
                x3_local_i = F.avg_pool2d(x3_local_i, x3_local_i.size()[2:]).view(x3_local_i.size(0), -1) #Global pooling for each local
                local_1024[:,region_idx] = x3_local_i


                x4_local_i = x4_local_list[region_idx]
                x4_local_i = F.avg_pool2d(x4_local_i, x4_local_i.size()[2:]).view(x4_local_i.size(0), -1) #Global pooling for each local
