...
def imgResize(img_root_dir, resume_point):
    failure = []
    img_file_list = os.listdir(img_root_dir)

    if(resume_point != "n"):
        k = img_file_list.index(resume_point)
        img_file_list = img_file_list[k: ]
        ...
        for f in files:
            ...
            if ext in ['.jpg', '.png']:
                try:
                    ...
                    print(menu +"-"+f+"Resizing 실패.")
                    failure.append(menu + "-" + f)
...
def checkRoot(file_path):
    ...
    resume = input("첫 폴더부터 시작 : 1 \n중간부터 시작 : 2\n ")
    if ans.lower() == "n":
        sys.exit()

    else:
        if resume == "2":
            foldername = input("중간부터 시작할 폴더 이름을 알려주세요. ")
            return foldername
        else:
            return "n"
...
if command.lower() != "resize":
    ...
    resume_point = checkRoot(sys.argv[2])
    failure_list = imgResize(img_root_dir, resume_point)

if len(failure_list) > 0:
    with open("failure_list.txt", mode = "wt", encoding= "utf-8") as o:
        o.write("--- 업로드 실패한 폴더 리스트 ---\n")
        for file in failure_list:
            o.write(file +"\n")
